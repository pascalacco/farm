...
void  AUDIO_OUT_ChangeBuffer(uint8_t *pData, uint16_t Size);
...
static int8_t AUDIO_AudioCmd_FS(uint8_t* pbuf, uint32_t size, uint8_t cmd)
{
  ...
  switch(cmd)
  {
    ...
    case AUDIO_CMD_PLAY:
        if (size == 0U)
        {
          AUDIO_IN_ChangeBuffer((uint8_t *)pbuf, AUDIO_TOTAL_BUF_SIZE);
        }
        else
        {
          AUDIO_IN_ChangeBuffer((uint8_t *)pbuf, 2*size);
        }
    break;
  }  
  return (USBD_OK);
